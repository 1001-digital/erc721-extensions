...
describe("RandomlyAssigned", async function () {
  ...
  it("Deployment should set the specified max supply", async function () {
    const contract = await viem.deployContract("RandomlyAssignedExample", [20n, 1n]);

    assert.equal(await contract.read.totalSupply(), 20n);
  });

  it("Mints all tokens in random order", async function () {
    const contract = await viem.deployContract("RandomlyAssignedExample", [20n, 1n]);
    ...
    assert.notDeepEqual(tokenIDs, incrementingTokenIds);
  });
  ...
  it("Works when minting multiple tokens within the same transaction", async function () {
    ...
    const hash = await contract.write.mint([20n], { account: buyerWallet.account });
    const receipt = await publicClient.getTransactionReceipt({ hash });
    const logs = parseEventLogs({
      abi: contract.abi,
      logs: receipt.logs,
